...
const Inbox: React.FC<InboxProps> = ({ currentUserNumber, onClose, onStartChat, onViewProfile, onAddFriend }) => {
    ...
    useEffect(() => {
        ...
        // 1. Load Base Contacts from Local Storage (Primary Source)
        const storedFriends = JSON.parse(localStorage.getItem('wa_friends') || '[]');
        
        // Initialize list with friends + Meta AI
        const initialList: ChatPreview[] = [
            {
                number: 'meta_ai',
                name: 'Meta AI Assistant',
                lastMessage: 'Ask me anything...',
                timestamp: Date.now(), // Keep it relatively new
                unread: 0,
                isAI: true
            },
        ...
        // 2. Setup Real-time Firebase Listeners for each friend
        const listeners: (() => void)[] = [];

        storedFriends.forEach((friend: any) => {
            // Sort numbers to generate consistent Chat ID
    ...
    return (
        <div className="fixed inset-0 bg-white z-40 flex flex-col animate-in slide-in-from-right duration-300">
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar">
                {loading ? (
                    <div className="flex justify-center p-10"><i className="fas fa-circle-notch animate-spin text-wa-light"></i></div>
                ) : chats.length === 0 ? (
                    <div className="text-center p-10 text-gray-500 mt-10">
                        ...
                        <button onClick={onAddFriend} className="text-emerald-600 font-bold">Add a friend</button>
                    </div>
                ) : (
                    chats.map((chat) => (
                        <div 
                        ...
                        >
                            ...
                            >
                                ...
                                <div className="flex justify-between items-center mt-0.5">
                                    <p className="text-sm text-gray-500 truncate pr-4 flex-1">
